...
const ensureTeacherIdColumn = async (tableName: 'batches' | 'live_classes') => {
    ...
    if (availableColumns.has('FacultyId')) {
        ...
        const quotedTeacherId = queryInterface.quoteIdentifier('teacherId');
        const quotedFacultyId = queryInterface.quoteIdentifier('FacultyId');
        await sequelize.query(
            `UPDATE ${quotedTable} SET ${quotedTeacherId} = COALESCE(${quotedTeacherId}, ${quotedFacultyId}) WHERE ${quotedTeacherId} IS NULL AND ${quotedFacultyId} IS NOT NULL`
        );
    }
