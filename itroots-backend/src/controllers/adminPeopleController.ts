...
export const createStudent = async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.status(statusCode).json({ message: getReadableErrorMessage(error, 'Server error during student creation') });
    }
};
...
export const assignStudentBatch = async (req: Request, res: Response) => {
    try {
        ...
        const statusCode = error instanceof ValidationError ? 400 : 500;
        res.status(statusCode).json({ message: getReadableErrorMessage(error, 'Server error during student assignment') });
    }
};
...
export const createFaculty = async (req: Request, res: Response) => {
    ...
    try {
        ...
        await transaction.rollback();
        const statusCode = error instanceof ValidationError ? 400 : 500;
        res.status(statusCode).json({ message: getReadableErrorMessage(error, 'Server error during Faculty creation') });
    }
};
...
export const assignFacultyResources = async (req: Request, res: Response) => {
    try {
        ...
        const statusCode = error instanceof ValidationError ? 400 : 500;
        res.status(statusCode).json({ message: getReadableErrorMessage(error, 'Server error during Faculty assignment') });
    }
};
...
export const sendUserWelcomeMail = async (req: Request, res: Response) => {
    try {
        ...
        const statusCode = error instanceof ValidationError ? 400 : 500;
        res.status(statusCode).json({ message: getReadableErrorMessage(error, 'Server error while sending welcome mail') });
    }
};
...
export const getStudentPayments = async (req: Request, res: Response) => {
    try {
        ...
        const statusCode = error instanceof ValidationError ? 400 : 500;
        res.status(statusCode).json({ message: getReadableErrorMessage(error, 'Server error fetching student payments') });
    }
};
